...
// Set stop loss according to cloud
   for(int idx = 0; idx < PositionsTotal(); idx++) {
      ...
      PositionSelectByTicket(PositionGetTicket(idx));

      //trade.PositionModify(PositionGetTicket(idx), FindStopLossBuy(PositionGetTicket(idx)), 0.0);
      if (CurrentTenkan() <= CurrentKijun()
            && Latest_Price.ask <= PositionGetDouble(POSITION_PRICE_OPEN)) {
         trade.PositionClose(PositionGetTicket(idx));
      }
   }
}
//+------------------------------------------------------------------+
...
void TakeProfit() {
   ...
   for(int idx = 0; idx < PositionsTotal(); idx++) {
      ...
      if (0 == 0
            && Latest_Price.ask > price_open + (100 * Point())
...
void GalaxyBuy() {
   if (PositionsTotal() < 1
      ...
      MqlTick Latest_Price; // Structure to get the latest prices
      SymbolInfoTick(Symbol(), Latest_Price); // Assign current prices to structure
      trade.Buy(INPUT_LOT, INPUT_SYMBOL, 0.0, Latest_Price.bid - (150 * Point()), 0.0, "Galaxy Buy");
      isBuy = false;
   }
}
//+------------------------------------------------------------------+
